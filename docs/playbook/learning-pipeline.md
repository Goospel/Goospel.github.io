...
**즉시 옮기지 않고 묶는 이유**: 매번 push / mkdocs nav 수정 / GitHub Actions 1-2분 빌드 / 카테고리 재정렬 — 마찰이 누적. 3-5개씩 묶으면 한 번에 끝. 게다가 동시 작성 시 서로 cross-link 자연스럽게 박힘.

#### 축 1. 타이밍 — 언제 batch 를 띄울 것인가

| 트리거 | 의미 | 빈도 |
|---|---|---|
| **Release PR (dev → main) 머지 직후** | "운영 의미 있는 마디" — 그 PR 의 학습이 운영 반영과 동시에 정리 | 가장 자연스러움 |
| **누적 3~5개 새 항목** | 1개씩은 비효율 / 너무 자주는 사이트 history 노이즈 | 자주 작업하는 시기 |
| **외부 데드라인 직전** — 면접 / 포트폴리오 공유 | 강제 데드라인 | 드물지만 강력 |
| **비활동 기간 끝** — 1주 / 1달 sweep | 백그라운드 누적 처리 | 작업 뜸한 시기 |

#### 축 2. 항목 — 무엇이 승격 자격이 있는가 (4문 다 통과)

| 질문 | 통과 신호 |
|---|---|
| 1. 일반화 가능한가? | 프로젝트 고유 ID / PR 번호 / 운영 URL 제거해도 본질이 남는가 |
| 2. 본인 이해 확립 완료? | 면접에서 본인 표현으로 설명 가능한가 — "아직 어색" 이면 vault 로 |
| 3. 같은 스택 누구나 만날 패턴? | 우연한 단발 사건 X, 일반 패턴 ✅ |
| 4. 검색했을 때 의미 있는 키워드? | 제목만 보고 "이거 내가 찾던 거" 느낌이 드는가 |

#### 승격 보류 — vault / 프로젝트 only

| 케이스 | 위치 |
|---|---|
| 너무 프로젝트 specific (예: "PR #62 ai_response UNIQUE 누락 사후 검증") | `learning-notes.md` 만 |
| 본인이 아직 어색 — 면접에서 더듬을 항목 | `learning-vault/half-baked/` |
| 일회성 trap — "이 사고 났을 때 이렇게 풀었다" | `troubleshooting.md` (T-###) |
| 회고성 — "왜 이게 헷갈렸나" | `learning-vault/reflections/` |

#### 의사결정 트리

```
새 learning-notes 항목 박힘
   │
   ├─→ "4문 다 통과?" → 아니면 vault / 프로젝트 only
   │
   ├─→ 통과 + 누적 1개 → 보류 (다음 항목 대기)
   │
   ├─→ 통과 + 누적 3~5개 → 묶어서 승격
   │
   └─→ 통과 + 외부 데드라인 / release 머지 시점 → 즉시 승격 (개수 무관)
```

빠른 휴리스틱 한 줄:
> "이걸 지금 옮기는 마찰 (5분) vs 다음 새 항목 1개 더 누적 + 한 번에 옮기는 마찰 (5분)" — 누적 1개면 보류, 3개면 즉시.
